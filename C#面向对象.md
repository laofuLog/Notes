...
> `&&`：如果表达式$1$位`false`，就不会对表达式$2$进行判断，直接返回`false`；所以`&&`比`&`效率高
>
...
​	3.`Parse`转换：基本数据类型可以用`Parse`转换，且字符串的值必须为可转数字
...
```
### **break&countinue关键字**
**break关键字**, 当执行到`break`时,会跳出`break`所在的循环块
...
`private sealed void Speak() `-->此时`Speak`方法无法被重写
